...
public final class PdaxAdapters {
  ...
  private static Order adaptOrder(PdaxOrder openOrder) {
    ...
    CurrencyPair currencyPair =
        adaptCurrencyPair(
            openOrder.getData().getTradeCurrency() + openOrder.getData().getSettlementCurrency());
    ...
    //		if (openOrder.isStop()) {
    //			order = new StopOrder(orderType, openOrder.getAmount(), currencyPair,
    // String.valueOf(openOrder.getId()),
    //					openOrder.getCreatedAt(), openOrder.getStopPrice(), openOrder.getPrice(),
    //					openOrder.getFieldCashAmount().divide(openOrder.getFieldAmount(), 8,
    // BigDecimal.ROUND_DOWN),
    //					openOrder.getFieldAmount(), openOrder.getFieldFees(),
  ...
  private static OrderStatus adaptOrderStatus(String orderStatus) {
    ...
    if (orderStatus.equals("PENDING_SUBMISSION")) return OrderStatus.PENDING_NEW;

    if (orderStatus.equals("ACTIVE")) return OrderStatus.PARTIALLY_FILLED;

    if (orderStatus.equals("EXPIRED")) return OrderStatus.EXPIRED;

    if (orderStatus.equals("PARTIAL_FILL")) return OrderStatus.PARTIALLY_FILLED;

    if (orderStatus.equals("FULL_FILL")) return OrderStatus.FILLED;

    if (orderStatus.equals("NO_FILL")) return OrderStatus.CANCELED;
